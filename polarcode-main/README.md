...
- 项目级记忆通过 `PAI.md` / `.polarcode/PAI.md` 启动自动注入，适合提交到仓库的团队共享规则；`PAI.local.md` / `.polarcode/PAI.local.md` 只做本地覆盖
...
- 配置持久化到 `~/.polarcode/config.json`，API Key 可从配置、环境变量或 `.env` 读取
...
- 新增 `com.polarcode.mcp` 模块，支持 stdio 子进程 server 与 Streamable HTTP 远程 server
- 启动时读取 `~/.polarcode/mcp.json` 与 `.polarcode/mcp.json`，项目级配置按 server 名覆盖用户级配置
...
- `~/.polarcode/mcp.json` 不存在时会自动创建默认 chrome-devtools 配置；项目级 `.polarcode/mcp.json` 仍可按 server 名覆盖
...
- `~/.polarcode/mcp.json` 不存在时启动自动创建模板，默认使用 `--isolated=true` 临时浏览器 profile
...
- shared 模式下 `close_page` 只能关闭 PolarCode 自己创建的 tab；无法证明是 PolarCode 创建的 tab 会被策略层拒绝
...
- 三层加载位置（按优先级，后者整体覆盖同名 skill）：jar 内置 < 用户级 `~/.polarcode/skills/<name>/` < 项目级 `<project>/.polarcode/skills/<name>/`
- 启动期把启用 skill 的 `name` + `description` 注入三处 Agent 系统提示词索引段（启用上限 20 个，索引段 ≤ 4KB）
- 内置工具 `load_skill(name)`：LLM 在 system prompt 看到匹配 description 时主动调用，PolarCode 把 SKILL.md 正文（5KB 截断）写入 `SkillContextBuffer`，下一轮 user message 自动前置注入
...
- 启用状态持久化：`~/.polarcode/skills.json` 的 `disabled` 列表，默认全启用
- 与 HITL 协同：Skill 内调用 `execute_command` 等危险工具仍走既有 HITL 审批，沿用 `execute_command` 工具维度全放行；不给 Skill 单独审批维度

设计意图：从「写工具」演进到「打包专家手册」。当工具堆成山（PolarCode 当前内置 9 个 + MCP 60+ 工具），用 Skill 给 LLM 一份按场景展开的"专家手册"，比往 system prompt 里塞更多规则更可扩展。
...
| 形态 | 启用方式 | 视觉风格 |
|---|---|---|
| **inline 流式 TUI**（默认） | 直接运行 / `POLARCODE_RENDERER=inline` | Claude Code / Qoder 风格：π 主题彩色开屏、主屏直出、transcript 当前位置的 `* ` 输入提示、JLine `Status` 托管的底部 dock（YOLO/HITL、MCP、Skill、model、ctx、token、cwd 等关键字段带克制彩色高亮；ctx 是当前上下文估算，in/out/cache 是调用统计）、右侧输入提示、行内可折叠工具块（`Read 3 files (ctrl+o to expand)`）、行内 git diff、HITL 单字符 `[y/n/a/s/m]` 提示 |
| **lanterna 全屏 TUI** | `POLARCODE_RENDERER=lanterna`（或兼容旧 `POLARCODE_TUI=true`） | v16 三栏全屏：文件树 + 对话流 + 状态栏 + 底部输入栏，HITL 模态弹窗 |
| **plain 兜底** | `POLARCODE_RENDERER=plain` | 纯 println，无折叠 / 状态栏，等价 v15 行为 |
...
- 对话历史保存到 `~/.polarcode/history/session_*.jsonl`
- 兼容旧设置：`POLARCODE_TUI=true` 自动映射为 `POLARCODE_RENDERER=lanterna`（已 deprecated）
- `POLARCODE_NO_STATUSBAR=true` 在 inline 模式下禁用 JLine 底部 dock（不适合 ANSI 光标控制的终端）
...
- 配置：`POLARCODE_LSP_ENABLED=false` 可关闭，`POLARCODE_LSP_MAX_DIAGNOSTICS=20` 可调整注入上限
...
- 快照仓库使用 JGit 纯 Java 实现，默认位于 `~/.polarcode/snapshots/<project_hash>/<worktree_hash>/.git`，不写用户项目 `.git`
...
- 配置：`POLARCODE_SNAPSHOT_ENABLED=false` 可关闭，`POLARCODE_SNAPSHOT_MAX=50`、`POLARCODE_SNAPSHOT_EXCLUDES=...`、`POLARCODE_SNAPSHOT_DIR=...` 可调整策略
...
- 支持用户级覆盖 `~/.polarcode/prompts/...`，支持项目级覆盖 `.polarcode/prompts/...`，项目级优先级最高
- 覆盖是整文件替换；`base.md` 和最终 prompt 必须包含 `## Language`
- Prompt 改动审计模板见 `docs/prompt-analysis-template.md`

### 第二十期：异步后台任务 + Runtime API（MVP）

- `DurableTaskManager` 使用 SQLite 持久化后台任务队列，默认位置 `~/.polarcode/tasks/tasks.db`
- 任务生命周期：`enqueued -> running -> completed / failed / canceled`
- `/task`、`/task add <任务内容>`、`/task cancel <task_id>`、`/task log <task_id>` 提供 CLI 闭环
- Worker Pool 默认 2 个后台 worker，可通过 `POLARCODE_TASK_WORKERS` 调整
- `java -jar target/polarcode-1.0-SNAPSHOT.jar serve --http --port 8080` 启动 localhost Runtime API
- Runtime API 端点：`POST /v1/threads`、`POST /v1/threads/{id}/turns`、`GET /v1/threads/{id}/events`
- Runtime API 强制要求 `POLARCODE_RUNTIME_API_KEY` 或 `-Dpolarcode.runtime.api.key`
...
- 新增进程级入口：`polarcode wechat setup`、`polarcode wechat start`、`polarcode wechat status`、`polarcode wechat daemon start|stop|restart|status|logs`
- 新增交互式入口：在 PolarCode 主界面输入 `/wechat` 可扫码绑定并在当前进程后台启动微信通道；`/wechat setup` 重新扫码绑定，`/wechat status` 查看状态，`/wechat stop` 停止通道
...
- 微信侧用户消息会回显到 PolarCode 终端 transcript；PolarCode 终端继续显示 thinking / 工具调用过程，微信侧只接收 assistant 正文。iLink 协议层仍是 `text_item.text` 文本消息，没有显式 Markdown parse mode；PolarCode 会保留 ClawBot 稳定支持的 Markdown 子集（列表、引用、粗体、行内代码、真实代码块），把标题转成粗体标题、把表格转成移动端更稳的键值/列表，并过滤图片 Markdown / H5-H6 / 中文斜体等兼容性差的标记；非代码类 fenced block（流程说明、长中文箭头链）会解包并换行，避免微信侧出现横向滚动代码块。iLink 不提供真正 SSE 或改单条消息能力。
...
`com.polarcode.policy` 包，作为 HITL 之外的辅助层（不是沙箱、不提供进程隔离）：
...
- `AuditLog` 结构化审计：危险工具调用按天写 JSONL 到 `~/.polarcode/audit/`，含 `outcome (allow|deny|error)` 与 `approver (hitl|policy|none)`；`revert_turn` 也纳入危险工具链
- `write_file` 单文件 5MB 上限
- CLI 命令：`/policy` 查看安全策略状态、`/audit [N]` 看最近 N 条审计

**为什么不叫沙箱**：本地 Agent CLI（参考 Claude Code / Cursor / Aider）默认都不做容器/VM 沙箱——沙箱削弱 Agent 能力、给虚假安全感、体验更差。生产级 Agent 沙箱实际是 microVM-level（Devin / Modal / Anthropic Computer Use 用 Firecracker / gVisor）。PolarCode 的安全模型是 **HITL + 路径校验 + 命令快速拒绝 + 审计**，不是隔离。
...
- 💾 默认模型持久化到 `~/.polarcode/config.json`
...
- 📋 结构化审计：危险工具调用按天写一行 JSONL 到 `~/.polarcode/audit/`，可通过 `/audit [N]` 查看
...
- `polarcode wechat setup` - 绑定微信 iLink 通道，选择 workspace 并完成扫码确认
- `polarcode wechat start` - 前台启动微信通道
- `polarcode wechat status` - 查看绑定状态和 daemon pid
- `polarcode wechat daemon start|stop|restart|status|logs` - 管理本机微信通道后台进程

交互式斜杠命令：

- `/wechat` - 扫码绑定并启动微信 iLink 通道；已绑定时直接启动
- `/wechat setup` - 重新扫码绑定并启动微信通道
- `/wechat status` - 查看当前 PolarCode 进程内微信通道状态
- `/wechat stop` - 停止当前 PolarCode 进程内微信通道
