...
```bash
cd /Users/itwanger/Documents/GitHub/polarcode
mvn -q clean package -DskipTests
```

产物：`target/polarcode-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
**预期**：
- Banner 显示简洁的 `PolarCode v16.1.0` 开屏，不带右侧盒线边框
...
```bash
POLARCODE_RENDERER=lanterna java -jar target/polarcode-1.0-SNAPSHOT.jar
```
...
## 11. 形态切换 — 兼容旧 POLARCODE_TUI（用例 10）
...
**预期**：
- stderr 出现 `⚠️ POLARCODE_TUI=true 已废弃，请改用 POLARCODE_RENDERER=lanterna`
...
```bash
POLARCODE_RENDERER=plain java -jar target/polarcode-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/polarcode-1.0-SNAPSHOT.jar
```
...
```bash
POLARCODE_RENDERER=weird java -jar target/polarcode-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 POLARCODE_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/polarcode-1.0-SNAPSHOT.jar
```
...
```
*
 PolarCode  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
**输入**：
```
把 ROADMAP.md 第一行改成 "# PolarCode 路线图（v16.1 测试）"
```
...
📝 ROADMAP.md
@@ -1,N +1,N @@
- # PolarCode ROADMAP（原标题）
+ # PolarCode 路线图（v16.1 测试）
  ...（上下文行）
```
...
│   [6] 配置文件: ~/.polarcode/config.json (只读视图，编辑请用编辑器)
...
## 23. POLARCODE_NO_STATUSBAR 禁用状态栏

```bash
POLARCODE_NO_STATUSBAR=true java -jar target/polarcode-1.0-SNAPSHOT.jar
```
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `POLARCODE_NO_STATUSBAR=true` 即可
