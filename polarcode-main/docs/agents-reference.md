...
1. `~/.polarcode/config.json` 中对应 provider 的 `apiKey`
...
| 数据 | 默认路径 | 覆盖方式 |
|------|----------|----------|
| 长期记忆 | `~/.polarcode/memory/long_term_memory.json` | `-Dpolarcode.memory.dir` |
| 项目级记忆 | `PAI.md` / `.polarcode/PAI.md` / `PAI.local.md` | 用户级稳定偏好：`~/.polarcode/PAI.md` |
| RAG 索引 | `~/.polarcode/rag/codebase.db` | `-Dpolarcode.rag.dir` |
| 审计日志 | `~/.polarcode/audit/audit-YYYY-MM-DD.jsonl` | `POLARCODE_AUDIT_DIR` / `-Dpolarcode.audit.dir` |
| Side-Git 快照 | `~/.polarcode/snapshots/<project_hash>/<worktree_hash>/.git` | `POLARCODE_SNAPSHOT_DIR` / `-Dpolarcode.snapshot.dir` |
| 后台任务 | `~/.polarcode/tasks/tasks.db` | — |

### Snapshot Config

系统属性 > 环境变量 > 默认值：`polarcode.snapshot.enabled`(true) / `polarcode.snapshot.max`(50) / `polarcode.snapshot.excludes`(.git,.polarcode/snapshots,target,node_modules,dist,.idea,*.class,*.jar) / `polarcode.snapshot.dir`(~/.polarcode/snapshots)
...
系统属性 > 环境变量/.env > 默认值：`POLARCODE_LOG_DIR`(~/.polarcode/logs) / `POLARCODE_LOG_LEVEL`(INFO) / `POLARCODE_LOG_MAX_HISTORY`(7) / `POLARCODE_LOG_MAX_FILE_SIZE`(10MB) / `POLARCODE_LOG_TOTAL_SIZE_CAP`(100MB)

### ReAct/SubAgent Budget Config

系统属性 > 默认值：`polarcode.react.token.budget`(Integer.MAX_VALUE) / `polarcode.react.stagnation.window`(3) / `polarcode.react.hard.max.iterations`(50)
...
系统属性 > 默认值：`polarcode.llm.connect.timeout.seconds`(60) / `polarcode.llm.read.timeout.seconds`(300) / `polarcode.llm.write.timeout.seconds`(60) / `polarcode.llm.call.timeout.seconds`(600)
...
1. 用户级：`~/.polarcode/mcp.json`
2. 项目级：`.polarcode/mcp.json`
...
- 加载顺序：`~/.polarcode/PAI.md` → `PAI.md` → `.polarcode/PAI.md` → `PAI.local.md` → `.polarcode/PAI.local.md`
...
- shared 模式 close_page 只允许关闭 PolarCode 创建的 tab

### Skill System

- 三层加载：jar 内置 < 用户级 ~/.polarcode/skills/ < 项目级 .polarcode/skills/
...
- 环境变量：`POLARCODE_RENDERER=inline|lanterna|plain`
- `POLARCODE_TUI=true`(旧) → lanterna + deprecation 提示
- `POLARCODE_NO_STATUSBAR=true`：禁用底部状态栏
...
- `/export` 导出当前 ReAct conversationHistory 为 Markdown 到 `~/.polarcode/exports/session-*.md`；包含完整 system prompt，便于检查 LLM 实际接收前的指令，命令不接受路径参数。
...
- `POLARCODE_LSP_ENABLED=false` 关闭

### Git Side-History Snapshot (Phase 18)

- side-git 在 ~/.polarcode/snapshots/ 维护独立仓库（JGit，不依赖系统 git）
...
- 覆盖优先级：jar 内置 < 用户级 ~/.polarcode/prompts/ < 项目级 .polarcode/prompts/
...
- XfyunMaaSClient：Qwen3.6-35B-A3B，默认 https://maas-api.cn-huabei-1.xf-yun.com/v2，OpenAI-compatible 讯飞星辰 MaaS；可用 `/config provider xfyun ...` 写入配置后 `/model xfyun` 切换。`model` 必须使用 MaaS 服务管控页展示的 modelId；微调模型可配置 `--lora-id <resourceId>`，作为 HTTP header `lora_id` 发出；该 provider 不发送 PolarCode 内置 tools。
...
# POLARCODE_SNAPSHOT_ENABLED=true
# POLARCODE_SNAPSHOT_MAX=50
# POLARCODE_SNAPSHOT_EXCLUDES=.git,.polarcode/snapshots,target,node_modules,dist,.idea,*.class,*.jar
# POLARCODE_SNAPSHOT_DIR=/Users/yourname/.polarcode/snapshots
# POLARCODE_TUI=true
# NO_TUI=true
```
