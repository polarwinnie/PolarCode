...
- 默认数据库：`~/.polarcode/tasks/tasks.db`
- 生命周期：
  - `enqueued`
  - `running`
  - `completed`
  - `failed`
  - `canceled`
- Worker Pool：默认 2 个后台 worker，可用 `POLARCODE_TASK_WORKERS` 或 `-Dpolarcode.task.workers` 覆盖
...
实现位于 `src/main/java/com/polarcode/runtime/api/`，使用 JDK 内置 `HttpServer`，不引入 Spring / Javalin。

启动：

```bash
POLARCODE_RUNTIME_API_KEY=your_local_api_key \
java -jar target/polarcode-1.0-SNAPSHOT.jar serve --http --port 8080
```

安全策略：

- 仅监听 `127.0.0.1`
- 必须配置 `POLARCODE_RUNTIME_API_KEY` 或 `-Dpolarcode.runtime.api.key`
- 请求头支持：
  - `Authorization: Bearer <key>`
  - `X-PolarCode-API-Key: <key>`
...
POLARCODE_RUNTIME_API_KEY=test java -jar target/polarcode-1.0-SNAPSHOT.jar serve --http --port 0
```
