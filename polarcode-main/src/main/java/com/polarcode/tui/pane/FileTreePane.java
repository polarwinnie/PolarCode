package com.polarcode.tui.pane;
...
public class FileTreePane extends Panel {
     ...
     * @param config 配置
     */
    public FileTreePane(PolarCodeConfig config) {
        super();
     ...
     * 加载忽略规则（用户级 + 内置默认）。
     */
    private static List<String> loadIgnorePatterns(PolarCodeConfig config) {
        List<String> patterns = new ArrayList<>(DEFAULT_IGNORE);

        // 读取用户级 filetree-ignore.txt
        Path userIgnore = Path.of(System.getProperty("user.home"), ".polarcode", "filetree-ignore.txt");
