package com.polarcode.snapshot;
...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("polarcode.snapshot.enabled", "POLARCODE_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("polarcode.snapshot.dir", "POLARCODE_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".polarcode", "snapshots").toString()));
        int max = readInt("polarcode.snapshot.max", "POLARCODE_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("polarcode.snapshot.excludes", "POLARCODE_SNAPSHOT_EXCLUDES", ""));
