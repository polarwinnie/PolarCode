package com.polarcode.mcp.transport;
...
public class StdioTransport implements McpTransport {
    ...
    public StdioTransport(String command, List<String> args, Map<String, String> env, Path workingDir) throws IOException {
        ...
        // Windows 上 ProcessBuilder 不会像 CMD 那样自动搜索 PATHEXT 扩展名，
        // npm 工具（npx, node, npm）实际是 .cmd 文件，不加扩展名会找不到。
        if (IS_WINDOWS && !command.contains(File.separator) && !command.contains(".")) {
            commandLine.add(command + ".cmd");
        } else {
    ...
    private void startStdoutReader() {
        Thread thread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                appendStderr("[polarcode] stdout reader stopped: " + e.getMessage());
            }
        }, "polarcode-mcp-stdio-stdout");
    ...
    private void startStderrReader() {
        Thread thread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                appendStderr("[polarcode] stderr reader stopped: " + e.getMessage());
            }
        }, "polarcode-mcp-stdio-stderr");
        thread.setDaemon(true);
        thread.start();
    }
