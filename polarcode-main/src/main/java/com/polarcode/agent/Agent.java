package com.polarcode.agent;

import com.polarcode.llm.LlmClient;
import com.polarcode.llm.LlmTraceLogger;
import com.polarcode.context.ContextProfile;
import com.polarcode.context.TokenUsageFormatter;
import com.polarcode.lsp.LspDiagnosticReport;
import com.polarcode.memory.ConversationHistoryCompactor;
import com.polarcode.memory.ExplicitMemoryHints;
import com.polarcode.memory.MemoryManager;
import com.polarcode.prompt.PromptAssembler;
import com.polarcode.prompt.PromptContext;
import com.polarcode.prompt.PromptMode;
import com.polarcode.prompt.ProjectMemoryLoader;
import com.polarcode.render.PlainRenderer;
...
import com.polarcode.skill.SkillIndexFormatter;
import com.polarcode.skill.SkillRegistry;
import com.polarcode.util.AnsiStyle;
import com.polarcode.tool.ToolRegistry;
import com.polarcode.tool.ToolRegistry.ToolExecutionResult;
import com.polarcode.tool.ToolRegistry.ToolInvocation;
import com.polarcode.util.TerminalMarkdownRenderer;
import com.polarcode.image.ImageReferenceParser;
...
public class Agent {
    ...
    public String run(String userInput) {
        ...
        while (true) {
            ...
            // 这是与Memory 短期记忆压缩并行的另一道压缩——后者只压 shortTermMemory，
    ...
    private String buildProjectMemoryContext() {
        try {
            ...
            log.warn("Failed to load POLAR.md project memory", e);
            return "";
        }
    }
    ...
    public String getContextStatus() {
        com.polarcode.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.polarcode.memory.TokenBudget.estimateMessagesTokens(java.util.List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
            return com.polarcode.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.polarcode.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.polarcode.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
                toolsSchemaTokens = com.polarcode.memory.MemoryEntry.estimateTokens(
