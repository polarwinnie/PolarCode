 ...
 * 1. 系统属性：{@code polarcode.react.token.budget} / {@code polarcode.react.stagnation.window} /
 *    {@code polarcode.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dpolarcode.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dpolarcode.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("polarcode.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("polarcode.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("polarcode.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
