...
class McpToolRegistrationTest {
    ...
    private static void withAuditDir(Path tempDir, ThrowingRunnable body) throws Exception {
        String previous = System.getProperty("polarcode.audit.dir");
        System.setProperty("polarcode.audit.dir", tempDir.resolve("audit").toString());
        try {
            body.run();
        } finally {
            if (previous == null) {
                System.clearProperty("polarcode.audit.dir");
            } else {
                System.setProperty("polarcode.audit.dir", previous);
            }
        }
    }
