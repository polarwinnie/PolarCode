...
class McpClientTest {
    ...
    void initializeTimeoutCanBeOverriddenBySystemProperty() {
        String previous = System.getProperty("polarcode.mcp.initialize.timeout.seconds");
        try {
            System.setProperty("polarcode.mcp.initialize.timeout.seconds", "17");
            assertEquals(17, McpClient.initializeTimeoutSeconds());
        } finally {
            if (previous == null) {
                System.clearProperty("polarcode.mcp.initialize.timeout.seconds");
            } else {
                System.setProperty("polarcode.mcp.initialize.timeout.seconds", previous);
            }
        }
    }
    ...
    void closeIsBestEffortAndDoesNotBlockOnServerSilence() throws Exception {
        // close 不再发 shutdown notification（server 卡死时会让 PolarCode 退出阻塞）。
